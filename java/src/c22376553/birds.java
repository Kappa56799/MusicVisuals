...
class Boid {
  ...
  void render() {
    ...
    // heading2D() above is now heading() but leaving old syntax until Processing.js
    // catches up
  ...
  void borders() {
    if (position.x < -r)
      position.x = mv.width + r;
    if (position.y < -r)
      position.y = mv.height + r;
    if (position.x > mv.width + r)
      position.x = -r;
    if (position.y > mv.height + r)
      position.y = -r;
  }
